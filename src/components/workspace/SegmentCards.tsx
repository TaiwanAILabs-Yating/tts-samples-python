...
import type { SegmentState, WordSegState } from "../../services/tts-orchestrator.ts";
...
interface SegmentCardItemProps {
  ...
  isActive: boolean;
  isPlaying: boolean;
  tailoEnabled: boolean;
  isWordSegAvailable: boolean;
  isRegenDisabled: boolean;
  canRegenerate: boolean;
  selectedSentenceIndex: number;
  onTogglePlay: (index: number, audio: ArrayBuffer) => void;
  onToggleTailo: (index: number) => void;
  onRegenerate: (index: number) => void;
  onSelect: (index: number) => void;
  onSaveText: (index: number, text: string) => void;
  onWordSegChange: (index: number, states: WordSegState[]) => void;
}

const SegmentCardItem = memo(function SegmentCardItem({
  segment,
  index: i,
  color,
  isActive,
  ...
  onSaveText,
  onWordSegChange,
}: SegmentCardItemProps) {
  const durStr = segment.duration != null ? `${segment.duration.toFixed(2)}s` : "--";
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Play */}
          <button
            onClick={(e) => { e.stopPropagation(); if (segment.audio) onTogglePlay(i, segment.audio); }}
          ...
          <button
            onClick={(e) => {
              ...
              a.href = url;
              a.download = `segment_${selectedSentenceIndex + 1}_${i + 1}.wav`;
              a.click();
      ...
      {tailoEnabled && (
        <WordSegmentation
          ...
          onWordStatesChange={(states) => onWordSegChange(i, states)}
        />
      )}
    </div>
  );
...
export function SegmentCards({
  ...
  // Stable callbacks so SegmentCardItem React.memo can short-circuit re-renders
  // for unaffected cards when activeSegmentIndex / playingIndex / tailoEnabledMap
  // changes for one card only.
  const handleToggleTailo = useCallback((i: number) => {
    setTailoEnabledMap((prev) => ({ ...prev, [i]: !prev[i] }));
  }, []);

  const handleRegenerate = useCallback(
    (i: number) => onRegenerateSegment(selectedIndex, i),
    [onRegenerateSegment, selectedIndex],
  );

  const handleSelect = useCallback(
    (i: number) => onSegmentClick?.(i),
    [onSegmentClick],
  );

  const handleSaveText = useCallback(
    (i: number, t: string) => updateSegmentText(selectedIndex, i, t),
    [updateSegmentText, selectedIndex],
  );

  const handleWordSegChange = useCallback(
    (i: number, states: WordSegState[]) =>
      updateSegmentWordSeg(selectedIndex, i, states),
    [updateSegmentWordSeg, selectedIndex],
  );
    ...
    <div className="flex flex-col gap-2 isolate">
      {segments.map((segment, i) => (
        <SegmentCardItem
          ...
          tailoEnabled={tailoEnabledMap[i] ?? false}
          isWordSegAvailable={isWordSegAvailable}
          isRegenDisabled={isRegenDisabled}
          canRegenerate={canRegenerate}
          selectedSentenceIndex={selectedIndex}
          onTogglePlay={togglePlay}
          onToggleTailo={handleToggleTailo}
          onRegenerate={handleRegenerate}
          onSelect={handleSelect}
          onSaveText={handleSaveText}
          onWordSegChange={handleWordSegChange}
        />
      ))}
    </div>
  );
