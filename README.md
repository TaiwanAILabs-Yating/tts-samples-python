...
- 支援多語言：中文、台語、日文、英文、韓文（日文 / 英文 / 韓文為 Beta）
...
- 批次匯出 ZIP：每個 Approved sentence 匯出為一個 `*_sentence_XXX.wav` + `metadata.json`（含生成參數、審核狀態、Notes）
- `Concat all sentences`：勾選後額外輸出全案合併的 `*_all_sentences.wav`（每個句子本來就會獨立匯出）

#### 文字輸入
- Direct Input：支援長文本，整段文字先切成 TTS segments，再每 50 個 segments 組成一個 workspace sentence
- Upload File：上傳 `.txt`，每個非空行視為一個獨立 sentence
- 分段預覽：生成前可預覽文字如何切分為 sentences / segments
- 字數驗證：Direct Input 總長上限 50,000 字；Upload File 每行上限 1,000 字、最多 200 句
...
#### 批次取代字詞
- 跨句子批次修改常見字詞（例如全部的「你」改成「汝」），不用逐張 Segment Card 手動編輯
- 即時預覽所有命中的 segment，取代前後對照，可逐項勾選要不要改
- 範圍可選「全部句子」或「目前句子」
- 「僅取代文字」或「取代並重新生成」二擇一；只更動文字，Tailo 斷詞狀態不受影響
...
- Tailo toggle：每個 Segment Card 可獨立開關斷詞面板，開啟後顯示 Word Chips
- 拼音選擇互動：點擊 chip 中文區域切回中文、點擊拼音區域啟用拼音替換、右鍵拼音開啟候選發音選單
...
| `VITE_ZERO_SHOT_API_URL` | TTS API endpoint | `https://ent.fedgpt.cc/api/asura/v1/speeches:zero-shot` |
| `VITE_PRESIGN_URL` | S3 presign endpoint | `https://ent.fedgpt.cc/api/asura/v1/transcriptions:presign` |
| `VITE_UPLOAD_URL` | Asset upload endpoint | `https://ent.fedgpt.cc/asset/` |
...
1. **上傳 Prompt Voice**
   ...
   - 可即時試聽已上傳的音檔
   - 選擇 Prompt 語言（中文 / 台語 / 日文 / 英文 / 韓文）
   - 輸入 Prompt 音檔對應的文字內容

2. **輸入文字**（Direct Input 與 Upload File 行為不同）

   **直接輸入模式：**
   - 在文字框直接輸入或貼上文字，換行會保留在同一篇長文本中
   - 系統會先依分段參數切成 TTS segments，再每 50 個 segments 組成一個 workspace sentence
   - 匯出時可勾選 `Concat all sentences`，將這些 workspace sentences 合成一個最終 WAV
   - 即時顯示字數與估算 token 數
   - 超過 50,000 字時會顯示警告
   ...
   **上傳檔案模式：**
   ...
   - 每個非空行視為一個獨立句子
   - 上傳後顯示檔名、行數、檔案大小
   - 超過 1,000 字/行或 200 句時會顯示警告

3. **設定參數**
   - 目標語言：中文 / 台語 / 日文 / 英文 / 韓文
   - 模型選擇：下拉自動抓取後端啟用中的 TTS model 清單（`type=tts`、`status=on`），亦可手動輸入 model ID
...
4. **批次取代字詞**
   - 入口：左側 Sidebar「Regenerate All / Approve All」右側的取代 icon 按鈕
   - 輸入「尋找 / 取代為」後即時列出所有命中的 segment（字面比對，不支援 regex）
   - 預覽列表顯示取代前後對照，預設全選，可逐項取消勾選
   - 「僅取代文字」：只改文字，之後自行重新生成
   - 「取代並重新生成」：改完文字後自動依序重新生成受影響的 segment（尚未生成過的句子只會改文字）
   - 已 Approved 的句子被取代後會退回 Generated，需重新審核

5. **審核**
   - 每句可 Approve（通過）或 Reject（退回）
   - 下方 Notes 欄位可記錄觀察（發音問題、斷句不佳等）
   - 「Approve All」可批次通過所有已生成的句子
   - Approve / Approve All 只改審核狀態，不會觸發 concat

#### Step 3：匯出

- **Segment 下載**：點擊各 segment 旁的下載按鈕，取得單一片段 WAV
- **單句下載**：Approve 後，可在句子 Header 點擊 Download 下載該句 WAV
- **批次匯出 ZIP**：
  - 永遠包含 `metadata.json`
  - 每個 Approved sentence 匯出為一個 `*_sentence_XXX.wav`；若該句尚未合併好（例如段落數超過自動合併門檻），下載前會即時把段落串成 sentence WAV
  - 勾選 `Concat all sentences` 時，會**額外**輸出 `*_all_sentences.wav`，把所有 Approved 句子串成單一音檔
  - Direct Input 預設勾選 `Concat all sentences`；Upload File 預設不勾選
...
2. **匯出含 Metadata 的 ZIP**
   - 在 Sidebar 底部點擊「Download Approved (ZIP)」
...
- **長文本限制**：
  ...
  - Direct Input 會被拆成多個 workspace sentences；這是為了讓每個 sentence 的 segment 數保持在可合併範圍內
  - 長文本下載時若勾選 `Concat all sentences`，瀏覽器需要額外時間與記憶體執行 FFmpeg 合併，請避免在合併中關閉分頁
