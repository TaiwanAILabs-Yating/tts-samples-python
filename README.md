...
- 批次匯出 ZIP：所有 Approved 音檔 + `metadata.json`（含生成參數、審核狀態、Notes）
- `Concat all sentences`：僅在下載 ZIP 時勾選，才合成全案單一 WAV
...
- **批次匯出 ZIP**：
  - 會匯出 `metadata.json`
  - 已有 sentence WAV 的短句會匯出為 `*_sentence_XXX.wav`
  - 未合併的長句若未勾選 `Concat all sentences`，會匯出各 segment WAV
  - 勾選 `Concat all sentences` 時，下載前才會合併所有 Approved 音訊，並輸出 `*_all_sentences.wav`
